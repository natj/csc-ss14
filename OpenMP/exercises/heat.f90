...
contains
  ...
  subroutine evolve(curr, prev, a, dt)
    ...
    ! finite-difference Laplacian with array syntax
...
!$omp parallel do shared(curr, prev, dt, a, nx, ny) private(i,j) &
!$omp collapse(2)
    do i = 2,nx-1
       do j = 2,ny-1
          curr%data(i,j) = prev%data(i,j) +dt*a* &
               ((prev%data(i-1,j) - 2.0*prev%data(i,j) + prev%data(i+1,j))/dx**2 + &
               (prev%data(i, j-1) - 2.0*prev%data(i,j) + prev%data(i, j+1))/dy**2)
       end do
    end do
!$omp end parallel do
