...
contains
  ...
  subroutine initialize(field0, parallel)
    ...
    win_size = (field0%nx+2)*(field0%ny+2) * double_size
    call mpi_win_create(field0%data, win_size,  &
         &              double_size, MPI_INFO_NULL, &
         &              parallel%comm, field0%rma_window, ierr)
    ...
    if (parallel % rank == 0) then
       ...
       field0%data(:,field0%ny+1) = -10.0_dp
    end if
  ...
  subroutine copy_fields(from_field, to_field, parallel)
    implicit none
    ...
    integer :: nx, ny, double_size, ierr
    integer(kind=MPI_ADDRESS_KIND) :: win_size
    ...
    if (.not.allocated(to_field%data)) then
       ...
       ! Set also the rma_window
       nx = ubound(from_field%data,1) - lbound(from_field%data,1)  
       ny = ubound(from_field%data,2) - lbound(from_field%data,2)  
       call mpi_type_size(MPI_DOUBLE_PRECISION, double_size, ierr)
       win_size = (to_field%nx+2)*(to_field%ny+2) * double_size
       call mpi_win_create(to_field%data, win_size,  &
         &                 double_size, MPI_INFO_NULL, &
         &                 parallel%comm, to_field%rma_window, ierr)
  ...
  subroutine exchange(field0, parallel)
    ...
    ! Put to right
    disp = 0
    call mpi_put(field0%data(:, field0%ny), field0%nx + 2, MPI_DOUBLE_PRECISION, &
                 parallel%nright, disp, field0%nx + 2, MPI_DOUBLE_PRECISION, &
                 field0%rma_window, ierr)
  ...
  subroutine output(curr, iter, parallel)
    ...
    if (parallel%rank == 0) then
       ...
       ! Send data
       tmp_data(:,:) = curr%data(1:curr%nx, 1:curr%ny)
  ...
  subroutine finalize(field0)
    ...
    call mpi_win_free(field0%rma_window, ierr)   
  ...
  subroutine read_input(field0, filename, parallel)
    ...
    integer :: nx, ny, i, ierr, double_size
    integer(kind=MPI_ADDRESS_KIND) :: win_size
