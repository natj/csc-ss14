...
{
	/* Write grid dimensions, iteration and the actual data from each
	 * process into a file using MPI I/O, so that one can read it back and
	 * continue the simulation from this saved position.
	 *
	 * Data to be saved:
	 *   parallel->nx_full
	 *   parallel->ny_full
	 ...
	 *   temperature->data
	 *
	 * Use the constant CHECKPOINT defined in the header file (.h) for the 
	 * output file name.
	 */

	// TODO: open file and write all the data into it
	MPI_File_open(parallel->comm, CHECKPOINT, ... );
}

void read_restart(field *temperature, parallel_data *parallel, int *iter)
{
	/* Read from a file the information saved by write_restart() to restore 
	 * the original state. Use MPI I/O to get the data directly to all 
	 * processes.
	 */
	...
	// TODO: open file and read grid size
	...
	...
	// TODO: read all the rest of the data
}
