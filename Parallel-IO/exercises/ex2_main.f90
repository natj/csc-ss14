...
program heat_solve
  ...
  if (restart) then
     call parallel_initialize(parallelization)
     call read_restart(current, previous, parallelization, step0)
     select case(command_argument_count())
     case(0) ! No arguments -> default values
     case(1) ! One argument -> number of steps to run
        using_input_file = .true.
        call get_command_argument(1, arg)
        read(arg,*) nsteps
     case default
        write (*,*) 'Only the number of time steps allowed as a command-line input when using restart'
        stop
     end select
  else
     step0 = 0
